...
template<typename T> typename List<T> operator+(const List<T>& left, const List<T>& right)
{
	List<T> cat = left;
...
void main()
{
	...
	for (List<double>::ReverseIterator rit = d_list.rbegin(); rit != d_list.crend(); ++rit)
		cout << *rit << tab;
	cout << endl;
